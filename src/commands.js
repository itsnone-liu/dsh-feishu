...
const HELP = [
  ...
  '- 订阅额度耗尽时桥会**自动等待恢复并继续**；配置了模型对时会**双向自动切换**：任一侧（GPT或GLM）额度满→自动切到另一侧续跑；**额度恢复不自动切回**（需换回手动 `/gpt` `/glm`）；两侧都满则只提示、不来回切',
  '- `/gpt` `/glm` `/auto` 手动快切模型：选择常态模型（启动/新建默认 GLM），手动选择不关闭额度安全网',
